...
class SinNeRF(LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=0):
        with torch.no_grad():
            if self.hparams.vit_weight > 0:
                refresh_each_step = getattr(self, 'refresh_vit_reference_each_step', False)
                if refresh_each_step or self.ref_ is None or (np.random.random() > 0.95):
                    self.ref_ = self.get_vit_feature(batch['real_patch']).detach()
